"""Transformer training and forecasting pipeline."""
...
from util.data_path import Transformer_err, Transformer_for, weights_path
...
def train_and_forecast(
    ...
    """Full Transformer pipeline: load data, train (or load weights), infer, save outputs.
    ...
    weight_file = weights_path / f"transformer_{crop_name}.pth"

    model = TransformerRegressor()
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(
        model.parameters(), lr=CFG["transformer"]["learning_rate"]
    )
    ...
    with torch.no_grad():
        ...
        tf_forecast_scaled = model(window).squeeze(0).numpy()

    tf_preds = scaler.inverse_transform(tf_forecast_scaled.reshape(-1, 1)).flatten()
    ...
    save_forecast_csv(forecast, Transformer_for, crop_name)
    ...
    save_error_csv(error_df, Transformer_err, crop_name)
    ...
    save_summary_csv(summary, Transformer_err, crop_name)
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="Train Transformer and forecast crop prices.")
