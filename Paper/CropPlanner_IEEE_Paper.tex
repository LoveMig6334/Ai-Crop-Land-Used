...
\begin{table*}[htbp]
  ...
  \begin{tabular}{lcccccl}
    ...
    \midrule
    Rohan et al.~\cite{rohan2025crop}      & LSTM, Trans.          & Single  & No  & No  & No  & General \\
  ...
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} ``Peak Timing'' indicates whether the work explicitly evaluates the accuracy of predicted peak price positions, distinct from overall forecast accuracy.
\end{table*}
...
\begin{table}[htbp]
  ...
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} ``YoY Std'' is the standard deviation of year-over-year percentage changes. ``Trend'' is the overall percentage change from 2004 to 2024.
\end{table}
...
The Transformer model~\cite{vaswani2017attention} uses multi-head self-attention to process the entire sequence simultaneously, efficiently capturing global dependencies across time steps. We adopt an encoder-only architecture rather than the full encoder--decoder design because our task is direct multi-step regression (12~inputs $\to$ 12~outputs via a linear head), not autoregressive sequence generation. Scalar monthly prices are first projected to the model dimension via a learned linear layer ($1 \to d_{\text{model}} = 64$), then augmented with sinusoidal positional encoding to preserve temporal ordering. The encoder consists of 2~layers, each with 8-head self-attention and a feed-forward network ($d_{ff} = 128$). The final hidden state is average-pooled across the sequence dimension and projected to 12~output values. Training uses MSE loss with the Adam optimizer (learning rate $= 0.001$) for 500~epochs, with the same MinMaxScaler preprocessing and full-batch training protocol as LSTM.
...
The application provides two main features: (1)~\textbf{Crop Prices}---displaying AI-predicted price charts for all four crops; and (2)~\textbf{Crop Planning}---accepting start month and area inputs, then presenting a Gantt chart recommendation with financial summaries (income, expenses, profit, and ROI). The interface was designed with simplicity as the primary concern, targeting non-technical users: price charts use familiar line-chart representations, and the planning page employs a Gantt chart---a visualization format commonly understood in agricultural extension contexts. The interface is responsive across desktop and mobile devices, ensuring accessibility for farmers in rural areas with limited computing resources.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for cassava (best peak timing). Peak correctly predicted in Jan (actual: Jan, predicted: Jan) with $\Delta$Month~=~0.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xticklabel style={font=\tiny}, yticklabel style={font=\tiny},
      xlabel style={font=\tiny}, ylabel style={font=\tiny},
      title={\scriptsize\bfseries Soybean --- RW-LSTM Forecast (2024)},
      xtick={1,2,...,12},
      xticklabels={J,F,M,A,M,J,J,A,S,O,N,D},
      ymin=13, ymax=28, grid=major, grid style={gray!15},
      legend style={font=\tiny, at={(0.98,0.98)}, anchor=north east},
    ]
    \addplot[blue, thick, mark=o, mark size=1.2pt] coordinates {
    ...
    % Peak markers
    \node[font=\tiny, blue, above] at (axis cs:4,21.67) {$\uparrow$Apr};
    \node[font=\tiny, red, above] at (axis cs:3,26.10) {$\uparrow$Mar};
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for soybean. The predicted peak (Mar) is one month before the actual peak~(Apr), achieving $\Delta$Month~=~1 despite significant price-level overestimation (MAPE~=~22.38\%).}
  \label{fig:rwlstm_soybean}
\end{figure}
...
Two of four crops (cassava and green bean) have exact peak month predictions, and soybean is within one month (Fig.~\ref{fig:rwlstm_soybean}). Corn's peak prediction is five months off, reflecting the difficulty of capturing its mid-year price spike with a short (6-year) window. Note that the $\Delta$Price column reflects overall forecast error at the peak month, not scheduling quality: the scheduling system uses only the peak month position to set the harvest date, while income is computed from the predicted price at that month. Cassava's high $\Delta$Price (29.57\%) despite exact peak timing indicates the forecast underestimates the price level but correctly identifies \textit{when} to harvest. Despite lower overall MAPE compared to the test-tuned configuration, the validated model retains strong peak timing for three of four crops---critical for the scheduling system.
...
% --- Fig: Cross-Model Comparison Grid ---
\begin{figure*}[htbp]
  \centering
  \subfloat[Cassava]{\includegraphics[width=0.45\textwidth]{model/error/views/cassava_models_comparison.png}\label{fig:cross_cas}}
  \hfill
  \subfloat[Corn]{\includegraphics[width=0.45\textwidth]{model/error/views/corn_models_comparison.png}\label{fig:cross_corn}}\\
  \subfloat[Green Bean]{\includegraphics[width=0.45\textwidth]{model/error/views/green_bean_models_comparison.png}\label{fig:cross_gb}}
  \hfill
  \subfloat[Soybean]{\includegraphics[width=0.45\textwidth]{model/error/views/soybean_models_comparison.png}\label{fig:cross_soy}}
  \caption{Cross-model comparison for all four crops. Each subplot shows actual prices, forecast curves, and per-month error analysis for LSTM, Transformer, and ARIMA models, revealing distinct strengths per crop.}
  \label{fig:cross_model}
\end{figure*}

An ensemble approach selecting the best-performing model per crop based on validation MAPE would achieve an estimated average test MAPE of approximately 5.76\% (LSTM for cassava at 4.81\%, LSTM for corn at 6.43\%, ARIMA for green bean at 6.08\%, Transformer for soybean at 5.71\%), compared to 7.07\% for the best single model (Standard LSTM). This represents an 18.5\% relative improvement and suggests that per-crop model selection is a promising direction for practical deployment.
...
Analysis of the 24~permutations reveals structural patterns in the scheduling problem. Orderings that place long-duration crops (cassava, 250~days) early tend to perform better because they constrain the timeline first, allowing shorter crops (green bean, 80~days; soybean, 90~days) to fill remaining gaps. Orderings that schedule multiple long-duration crops consecutively tend to produce exclusions, reducing total income. Specifically, of the 24~permutations, only 8 successfully schedule all four crops; the remaining 16 exclude at least one crop due to overlapping time constraints. The difference between the best and worst 4-crop orderings is approximately 15\%, while orderings that exclude crops can lose up to 45\% of the optimal income. This sensitivity to ordering motivates exact methods such as Dynamic Programming for production deployment.
...
First, a comparative study of four forecasting models---Standard LSTM, Transformer, ARIMA, and the proposed Rolling-Window LSTM---revealed that no single model dominates across all crops, with each achieving the best accuracy for at least one crop. Standard LSTM achieved the highest average accuracy (92.93\%), while Rolling-Window LSTM---with window sizes validated on a held-out year to prevent data leakage---retained accurate peak timing for three of four crops. Critically, we identified and documented three systematic limitations of standard models (off-peak prediction, long-range averaging, and structural break sensitivity) that motivate the windowed approach, and introduced peak timing accuracy as an application-aligned evaluation metric. Cross-model analysis further showed that an ensemble selecting the best model per crop could reduce average MAPE from 7.07\% to approximately 5.76\%.

Second, we formalized the crop planning problem as a Weighted Job Scheduling Problem with mathematical notation and complexity analysis. Exhaustive enumeration over all $4! = 24$ permutations identified the optimal non-overlapping four-crop rotation yielding 39,212~THB/rai/year---approximately 1.9~times the best monoculture income. The greedy heuristic, formalized in Algorithm~\ref{alg:greedy}, was suboptimal by 14.8\%, demonstrating that even with four crops the scheduling structure is non-trivial and motivating exact methods for production deployment. Sensitivity analysis confirmed robustness to uniform yield variability.

Third, the CropPlanner web application enables farmers to plan up to 12~months ahead through intuitive visualizations, delivering AI-powered insights without requiring technical expertise. The system demonstrates the feasibility of an end-to-end pipeline from multi-model forecasting through scheduling optimization to web-based delivery---a combination that, as shown in our literature review, has received limited attention in prior work. This system has the potential to reduce income volatility and promote sustainable, diversified agriculture for Thai farmers.
