...
\begin{figure*}[htbp]
  ...
  \begin{tikzpicture}
    \begin{groupplot}[
      group style={group size=2 by 2, horizontal sep=1.8cm, vertical sep=1.2cm},
...
The Transformer model~\cite{vaswani2017attention} uses multi-head self-attention to process the entire sequence simultaneously, efficiently capturing global dependencies across time steps. We adopt an encoder-only architecture rather than the full encoder--decoder design because our task is direct multi-step regression (12~inputs $\to$ 12~outputs via a linear head), not autoregressive sequence generation. Scalar monthly prices are first projected to the model dimension via a learned linear layer ($1 \to d_{\text{model}} = 64$), then augmented with sinusoidal positional encoding to preserve temporal ordering. The encoder consists of 2~layers, each with 8-head self-attention and a feed-forward network ($d_{ff} = 128$). The last time step's hidden state is extracted and projected to 12~output values via a linear layer. Training uses MSE loss with the Adam optimizer (learning rate $= 0.001$) for 500~epochs, with the same MinMaxScaler preprocessing and full-batch training protocol as LSTM.
...
\begin{table}[htbp]
  ...
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} ``Oracle'' selection uses the known best model per crop on the test set. In practice, selection would be based on validation performance, which may not always identify the test-set winner.
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{clccc}
    ...
    \midrule
    1  & Co$\to$GB$\to$So$\to$Ca & 4/4 & 41,267 & 100.0\% \\
    2  & GB$\to$Co$\to$So$\to$Ca & 4/4 & 41,267 & 100.0\% \\
    3  & Co$\to$GB$\to$Ca$\to$So & 4/4 & 39,757 & 96.3\% \\
    4  & GB$\to$Co$\to$Ca$\to$So & 4/4 & 39,757 & 96.3\% \\
    5  & Ca$\to$GB$\to$Co$\to$So & 4/4 & 39,211 & 95.0\% \\
    \midrule
    20 & So$\to$Co$\to$Ca$\to$GB & 3/4 & 29,790 & 72.2\% \\
    21 & So$\to$Co$\to$GB$\to$Ca & 3/4 & 29,790 & 72.2\% \\
    22 & So$\to$GB$\to$Co$\to$Ca & 3/4 & 29,790 & 72.2\% \\
    23 & So$\to$Ca$\to$Co$\to$GB & 2/4 & 15,380 & 37.3\% \\
    24 & So$\to$Ca$\to$GB$\to$Co & 2/4 & 15,380 & 37.3\% \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize Ca=Cassava, Co=Corn, GB=Green Bean, So=Soybean. Income in THB/rai/year.
\end{table}

Of the 24~permutations, 14~successfully schedule all four crops, 8~exclude one crop (3/4), and 2~exclude two crops (2/4). Two clear patterns emerge. First, orderings that place short-duration, high-flexibility crops first (corn at 120~days, green bean at 80~days) achieve higher incomes because they secure peak-price time slots without over-constraining the timeline, leaving room for cassava's long 250-day growing period via the previous-year strategy. The top~2 orderings both start with corn or green bean and end with cassava. Second, orderings starting with soybean consistently underperform---all bottom~5 entries begin with soybean, whose 90-day duration and March peak compete directly with corn's February peak, creating early conflicts that cascade into exclusions. The worst orderings (So$\to$Ca) lose 63\% of optimal income by scheduling only 2~crops. This sensitivity to ordering---even with just four crops---motivates exact methods such as Dynamic Programming for production deployment.
