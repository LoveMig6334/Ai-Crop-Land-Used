...
\begin{enumerate}
  ...
  \item \textbf{Crop planning recommendation:} We formulate the planting schedule optimization as a Weighted Job Scheduling Problem and solve it using a Greedy Heuristic that produces non-overlapping planting--harvest timelines.
...
The CropPlanner system comprises three components: (A)~AI price forecasting models, (B)~crop planning recommendation, and (C)~a web application. All models take 12~months of historical prices as input and output 12-month forecasts.
...
Long Short-Term Memory (LSTM)~\cite{hochreiter1997lstm} is a recurrent neural network designed to learn long-term dependencies via gating mechanisms (forget, input, and output gates). Our LSTM model consists of 2~layers with hidden size~32, receiving 12~months of input and producing 12~months of output. Training used MSE loss on PyTorch for 300~epochs with a learning rate of 0.01 and random seed~42.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[node distance=0.35cm]
    % Input
    \node[block=yellow!20, text width=1.6cm, align=center] (input) {Input\\$x \in \mathbb{R}^{12 \times 1}$};

    % Input projection
    \node[block=purple!15, below=0.4cm of input, text width=2cm, align=center] (proj) {Linear Projection\\$1 \to 64$};

    % Positional Encoding
    \node[block=cyan!15, below=0.4cm of proj, text width=2cm, align=center] (pe) {Positional\\Encoding};

    % Encoder block (repeated)
    \node[block=green!15, below=0.5cm of pe, text width=2.8cm, align=center, minimum height=1.4cm] (enc) {
      \begin{tabular}{c}
        \textbf{Encoder Layer} $\times 2$\\[2pt]
        Multi-Head Attention\\
        ($h=8$, $d_{\text{model}}=64$)\\[2pt]
        Feed-Forward ($d_{ff}=128$)\\
        + Add \& Norm
      \end{tabular}
    };

    % Last step
    \node[block=orange!20, below=0.4cm of enc, text width=1.6cm, align=center] (last) {Last Step\\$e_{T}$};

    % Decoder
    \node[block=red!15, below=0.4cm of last, text width=1.6cm, align=center] (dec) {Linear\\$64 \to 12$};

    % Output
    \node[block=blue!15, below=0.4cm of dec, text width=1.6cm, align=center] (output) {Output\\$\hat{y} \in \mathbb{R}^{12}$};
    ...
    \draw[-{Stealth[length=1.5mm]}, gray, dashed] (pe.east) -- ++(0.8,0) |- (enc.east);
  \end{tikzpicture}
...
To address these issues, we propose Rolling-Window LSTM. The 20-year dataset is divided into rolling windows of 7--8~years (crop-dependent), shifted by 1~year. A 3-layer LSTM (hidden size~32, 300~epochs) is trained on each window independently. Predictions from all windows are consolidated into a final forecast for the target year.
...
Rolling-Window LSTM forecasts are normalized by dividing each crop's price series by its mean, enabling cross-crop comparison. The peak normalized price date (argmax) defines the harvest date---the optimal selling time. The planting date is computed by subtracting the growing period:
\begin{equation}
...
\begin{figure*}[htbp]
  ...
  \begin{tikzpicture}[node distance=0.5cm and 0.6cm]
    ...
    % A success
    \node[process, above=0.5cm of stratA] (addA) {Add $j_i$ to $S$\\(original dates)};
    ...
    % B success
    \node[process, above=0.5cm of stratB] (addB) {Shift to gap;\\recalc income;\\add to $S$};
    ...
    % C success
    \node[process, above=0.5cm of stratC] (addC) {Plant in prev.\\year; harvest\\in 2024; add to $S$};
...
The optimized four-crop rotation yields approximately 2.4~times the income of the best monoculture option (corn).
...
First, a comparative study of four forecasting models revealed that no single model dominates. Standard LSTM achieved the highest average accuracy (92.93\%), while Rolling-Window LSTM exhibited superior peak timing accuracy---predicting peaks within one month for three of four crops.

Second, Greedy Scheduling produced a non-overlapping four-crop rotation yielding 41,377~THB/rai/year---approximately 2.4~times the best monoculture income.
