...
\begin{figure}[htbp]
  ...
  \subfloat[RW-LSTM (peak: Jul, actual: Sep, $\Delta$=1~mo.)]{%
  \begin{tikzpicture}
    \begin{axis}[
      width=\columnwidth, height=3.5cm,
      xlabel={\scriptsize Month (2024)}, ylabel={\scriptsize Price (THB/kg)},
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for cassava (best case). Peak predicted within one month (actual: Jan~3.65, predicted: Jan~2.57) with $\Delta$Month~=~1.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for corn (challenging case). The predicted peak (Feb) arrives five months before the actual peak~(Jul), with $\Delta$Month~=~3.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \def\mnames{{"A23","M23","J23","J23","A23","S23","O23","N23","D23","J24","F24","M24","A24","M24","J24","J24","A24","S24","O24","N24","D24"}}
    % Y positions
    ...
    \foreach \i in {0,3,...,18} {
      \draw (\i,{-0.7}) -- (\i,{-0.5});
      \pgfmathparse{\mnames[\i]}
      \node[font=\tiny, below] at (\i,{-0.7}) {\pgfmathresult};
    }
    % Crop labels
