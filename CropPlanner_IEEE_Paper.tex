...
\begin{figure}[htbp]
  ...
  \subfloat[RW-LSTM (peak: Sep, actual: Sep, $\Delta$=0~mo.)]{%
  \begin{tikzpicture}
    \begin{axis}[
      width=\columnwidth, height=3.5cm,
      xlabel={\scriptsize Month (2024)}, ylabel={\scriptsize Price (THB/kg)},
  ...
  \caption{Green bean 2024 forecast comparison. (a)~Standard LSTM produces a smooth curve peaking in Jul, missing the sharp Sep peak. (b)~RW-LSTM correctly predicts peak timing in Sep ($\Delta$=0~mo.) with the validated 5-year window.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for cassava (best case). Peak correctly predicted in Jan (actual: Jan, predicted: Jan) with $\Delta$Month~=~0.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \node[font=\tiny, red, above] at (axis cs:2,13.58) {$\uparrow$Feb};
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for corn (challenging case). The predicted peak (Feb) arrives five months before the actual peak~(Jul), with $\Delta$Month~=~5.}
...
\begin{figure}[htbp]
  ...
  \end{tikzpicture}}%
  \caption{Optimized Gantt chart (brute-force optimal ordering). Corn and soybean were shifted to available gaps using Strategy~B (dashed borders), producing a non-overlapping four-crop rotation with total income of 39,211~THB/rai/year.}
  \label{fig:gantt_opt}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    \textbf{Total} & \textit{Infeasible} & \textbf{39,211} & --- & --- \\
    \bottomrule
  \end{tabular}
\end{table}

Corn and soybean lost income from shifting to non-peak months, while the total feasible income reached 39,211~THB/rai/year. This schedule was identified through exhaustive enumeration (Section~\ref{sec:bruteforce}) rather than the greedy heuristic. Table~\ref{tab:mono} compares this with monoculture baselines.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    \textbf{4-Crop Rotation} & \textbf{39,211} & --- \\
    Mono: Corn (best) & 20,377 & $+$92.4\% \\
    Mono: Cassava & 8,998 & $+$335.8\% \\
    Mono: Soybean & 7,830 & $+$400.8\% \\
    Mono: Green Bean & 6,818  & $+$475.1\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
Exhaustive enumeration of all $4! = 24$ crop permutations was performed to verify the greedy heuristic. The brute-force search identified an optimal schedule yielding 39,211~THB/rai/year, compared to the greedy result of 33,394~THB/rai/year---an optimality gap of 14.8\%. The greedy heuristic, by prioritizing corn (highest income) first, blocked soybean from being scheduled. In contrast, the optimal ordering (Cassava~$\rightarrow$~Soybean~$\rightarrow$~Green Bean~$\rightarrow$~Corn) accommodates all four crops through strategic shifting. This result demonstrates that greedy scheduling is suboptimal even for four crops and motivates the use of exact methods such as exhaustive enumeration or Dynamic Programming for production deployment. Table~\ref{tab:income_detail} and Fig.~\ref{fig:gantt_opt} present the brute-force optimal schedule.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccc}
    ...
    \midrule
    1.1 ($+$10\%) & 43,132 & $\times$1.92 \\
    1.0 (baseline) & 39,211 & $\times$1.92 \\
    0.9 ($-$10\%)  & 35,290 & $\times$1.92 \\
    0.8 ($-$20\%)  & 31,369 & $\times$1.92 \\
    0.7 ($-$30\%)  & 27,448 & $\times$1.92 \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} Because Income~$=$~Yield~$\times$~Price, uniform scaling preserves the rotation-to-monoculture ratio ($\times$1.92) across all yield factors.
\end{table}
...
With only four crops, the scheduling problem admits $4! = 24$ permutations, making exhaustive search trivial. Our brute-force verification revealed that the greedy heuristic is suboptimal: it produced 33,394~THB/rai/year (3~crops, soybean excluded), compared to the optimal 39,211~THB/rai/year (all 4~crops)---a gap of 14.8\%. The greedy algorithm's income-first prioritization placed corn first, which consumed a time slot that prevented soybean from being scheduled. The optimal ordering prioritizes cassava first (long growing period, must start early), followed by crops that fill the remaining gaps. This demonstrates that even with four crops, the scheduling structure is non-trivial: greedy approaches can miss solutions that require counter-intuitive priority orderings. For larger crop sets ($>$10), exhaustive search becomes infeasible ($15! \approx 1.3 \times 10^{12}$), and Dynamic Programming in $O(n \log n)$ becomes necessary.
...
Second, exhaustive enumeration identified the optimal non-overlapping four-crop rotation yielding 39,211~THB/rai/year---approximately 1.9~times the best monoculture income. The greedy heuristic was found to be suboptimal by 14.8\%, demonstrating the value of exact methods even for small problem instances. Sensitivity analysis confirmed the rotation advantage is robust to uniform yield variability.
