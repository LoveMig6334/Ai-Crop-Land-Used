...
Data were split into a training set (2004--2023) and a test set (2024 only). The scaler was fit exclusively on training data to prevent data leakage. No separate validation set was used; hyperparameters were fixed \textit{a priori} and not tuned on test results.
...
\end{equation}
where $Y_i$ is yield (kg/rai), $P_i^{\text{peak}}$ is the price at peak (THB/kg), and $A$ is area. In this study, $A = 1$~rai for all computations.
...
A single test year (2024) may not capture model stability. The absence of a validation set risks undetected overfitting. While network hyperparameters (hidden size, learning rate, epochs) were fixed \textit{a priori}, the RW-LSTM window sizes (7~or 8~years) were selected through per-crop experimentation that involved feedback from the test set, constituting a form of hyperparameter tuning. Multi-year rolling evaluation with a held-out validation period would provide more robust conclusions.
...
This study has several limitations: (1)~a single test year (2024) with no validation set limits the generalizability of model comparisons; (2)~no field testing with actual farmers was conducted, leaving usability and adoption unverified; (3)~external factors such as weather variability, government policy changes, and global commodity prices are not incorporated into the forecasting models; (4)~the greedy scheduling heuristic does not guarantee global optimality; and (5)~coverage is limited to Northeastern Thailand and four field crops.

Future directions include: (1)~ensemble model selection per crop to leverage each model's strengths; (2)~multi-year rolling evaluation with a held-out validation set for robust comparison; (3)~incorporating weather, macroeconomic, and supply-chain features as additional input variables; (4)~Dynamic Programming for scheduling with $>$10 crops; (5)~user studies and pilot deployments with farmers to evaluate real-world impact; and (6)~expanding to additional regions and crop categories including horticultural crops.
