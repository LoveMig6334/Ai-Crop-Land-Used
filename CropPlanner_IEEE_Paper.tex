...
Resource allocation in agriculture can be modeled as optimization problems such as the Knapsack Problem for area allocation or Job Scheduling for temporal planning. Sarker and Ray~\cite{sarker2009crop} formulated crop planning as a multi-objective optimization model and demonstrated the use of evolutionary algorithms to identify Pareto-optimal planting configurations that balance net return against crop diversity. Their work highlights the applicability of combinatorial optimization to agricultural scheduling, though it does not incorporate price forecasting as a decision input.
...
The gap that this work addresses is the integration of AI price forecasting with scheduling optimization and farmer-accessible web delivery---a pipeline that has received limited attention in prior literature.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    % Shift annotation
    \draw[winbrace] ([yshift=-12pt]y0.south west) -- node[below=4pt, font=\tiny] {7--8 year window} ([yshift=-12pt]y7.south east);
...
To address these issues, we propose the Rolling-Window LSTM. The dataset is divided into rolling windows of 5--10~years (crop-dependent, selected via validation), each shifted by 1~year. A~3-layer LSTM (hidden size~32, 300~epochs) is trained on each window independently. Predictions from all windows are then consolidated into a final forecast for the target year.

Window sizes were selected via validation-set evaluation. For each crop, we swept window sizes from 5 to 10~years and selected the size yielding the lowest MAPE on the 2023 validation year. Table~\ref{tab:window_selection} presents the validation results. Cassava achieved the best validation MAPE with an 8-year window (31.25\%), corn with a 6-year window (11.43\%), while green bean and soybean performed best with 5-year windows (12.67\% and 15.62\%, respectively). The final models were then retrained on the combined training and validation data (2004--2023) using these selected window sizes and evaluated on the held-out test year (2024). By learning from multiple shorter windows, the model captures recurring seasonal patterns more accurately---particularly the location of peak prices, which is critical for the downstream scheduling system.
...
\begin{table}[htbp]
  ...
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} Bold values indicate the selected window size per crop (lowest validation MAPE).
\end{table}

\begin{figure}[htbp]
  \centering
  \subfloat[Standard LSTM (peak: Jul, actual: Sep, $\Delta$=2~mo.)]{%
  \begin{tikzpicture}
    \begin{axis}[
    ...
    \end{axis}
  \end{tikzpicture}\label{fig:lstm_gb}}\\[2pt]
  \subfloat[RW-LSTM (peak: Aug, actual: Sep, $\Delta$=1~mo.)]{%
  \begin{tikzpicture}
    \begin{axis}[
      width=\columnwidth, height=3.5cm,
      xlabel={\scriptsize Month (2024)}, ylabel={\scriptsize Price (THB/kg)},
    ...
    \addplot[orange, dashed, thick, mark=triangle*, mark size=1.2pt] coordinates {
      (1,23.75)(2,23.87)(3,23.93)(4,24.04)(5,24.15)(6,24.15)
      (7,24.18)(8,24.16)(9,23.95)(10,23.89)(11,23.87)(12,23.89)
    }; \addlegendentry{RW-LSTM}
    \end{axis}
  \end{tikzpicture}\label{fig:rwlstm_gb}}
  \caption{Green bean 2024 forecast comparison. (a)~Standard LSTM produces a smooth curve peaking in Jul, missing the sharp Sep peak. (b)~RW-LSTM captures the flat pre-peak region and predicts closer peak timing.}
...
The algorithm enforces four constraints: (1)~non-overlap---one crop per plot at any time; (2)~one planting per crop; (3)~harvest must fall within the target year; (4)~fixed area of 1~rai. As a greedy heuristic, it does not guarantee global optimality---scheduling a high-income crop first may block two medium-income crops with higher combined revenue. To verify solution quality, we additionally perform an exhaustive search over all $4! = 24$ permutations. With only four crops, exhaustive enumeration completes in negligible time, serving as an exact optimality benchmark. The greedy heuristic is presented as the general-purpose algorithm applicable to larger crop sets where exhaustive search becomes infeasible.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xticklabel style={font=\tiny}, yticklabel style={font=\tiny},
      xlabel style={font=\tiny}, ylabel style={font=\tiny},
      title={\scriptsize\bfseries Cassava --- RW-LSTM Forecast (2024)},
      xtick={1,2,...,12},
      xticklabels={J,F,M,A,M,J,J,A,S,O,N,D},
      ymin=2, ymax=4, grid=major, grid style={gray!15},
    ...
    % Peak markers
    \node[font=\tiny, blue, above] at (axis cs:1,3.65) {$\uparrow$Jan};
    \node[font=\tiny, red, below] at (axis cs:1,2.57) {$\uparrow$Jan};
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for cassava (best case). Peak predicted within one month (actual: Jan, predicted: Feb) with $\Delta$Month~=~1.}
  \label{fig:rwlstm_cassava}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xticklabel style={font=\tiny}, yticklabel style={font=\tiny},
      xlabel style={font=\tiny}, ylabel style={font=\tiny},
      title={\scriptsize\bfseries Corn --- RW-LSTM Forecast (2024)},
      xtick={1,2,...,12},
      xticklabels={J,F,M,A,M,J,J,A,S,O,N,D},
      ymin=8, ymax=15, grid=major, grid style={gray!15},
      legend style={font=\tiny, at={(0.98,0.98)}, anchor=north east},
    ]
    \addplot[blue, thick, mark=o, mark size=1.2pt] coordinates {
    ...
    \addplot[red, dashed, thick, mark=square*, mark size=1.2pt] coordinates {
      (1,13.45)(2,13.80)(3,13.30)(4,12.91)(5,12.38)(6,12.42)
      (7,11.71)(8,11.49)(9,11.10)(10,10.53)(11,9.83)(12,9.63)
    }; \addlegendentry{RW-LSTM}
    % Peak markers
    \node[font=\tiny, blue, above] at (axis cs:7,12.45) {$\uparrow$Jul};
    \node[font=\tiny, red, above] at (axis cs:2,13.80) {$\uparrow$Feb};
    \end{axis}
  \end{tikzpicture}
  \caption{RW-LSTM forecast for corn (challenging case). The predicted peak (Apr) arrives three months before the actual peak~(Jul), with $\Delta$Month~=~3.}
  \label{fig:rwlstm_corn}
\end{figure}
...
Two of four crops (cassava and green bean) have exact peak month predictions, and soybean is within one month. Corn's peak prediction is five months off, reflecting the difficulty of capturing its mid-year price spike with a short (6-year) window. Despite lower overall MAPE compared to the test-tuned configuration, the validated model retains strong peak timing for three of four crops---critical for the scheduling system where peak position determines harvest dates.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \addplot[orange, mark=o] coordinates {
      (1,1.103)(2,1.073)(3,1.046)(4,1.007)(5,0.948)(6,0.955)
      (7,0.944)(8,1.005)(9,1.011)(10,0.996)(11,0.968)(12,0.926)
    }; \addlegendentry{Cassava}
    % Corn (mean=11.88)
    \addplot[green!60!black, mark=square*] coordinates {
      (1,1.133)(2,1.162)(3,1.120)(4,1.087)(5,1.042)(6,1.045)
      (7,0.986)(8,0.967)(9,0.935)(10,0.887)(11,0.827)(12,0.811)
    }; \addlegendentry{Corn}
    % Green Bean (mean=23.99)
    \addplot[red, mark=triangle*] coordinates {
      (1,0.990)(2,0.995)(3,0.998)(4,1.002)(5,1.007)(6,1.007)
      (7,1.008)(8,1.007)(9,0.998)(10,0.996)(11,0.995)(12,0.996)
    }; \addlegendentry{Green Bean}
    % Soybean (mean=22.16)
    \addplot[blue, mark=diamond*] coordinates {
      (1,1.085)(2,1.190)(3,1.219)(4,1.042)(5,0.916)(6,0.914)
      (7,0.969)(8,1.065)(9,1.103)(10,0.998)(11,0.784)(12,0.717)
    }; \addlegendentry{Soybean}
    % Mean reference line
    \addplot[gray, dashed, thin] coordinates {(1,1.0)(12,1.0)};
    \end{axis}
  \end{tikzpicture}
  \caption{Normalized RW-LSTM forecasts (2024). Each crop's predicted prices are divided by their mean, enabling cross-crop comparison. Peaks occur in different months---Cassava (Jan), Corn (Feb), Green Bean (Jul), Soybean (Mar)---which enables non-overlapping crop rotation.}
  \label{fig:overlap}
\end{figure}

The price overlap analysis reveals that peak prices for the four crops occur in different months, enabling crop rotation. The unoptimized Gantt chart (Fig.~\ref{fig:gantt_unopt}), where each crop is placed at its peak-optimal position, shows that green bean and corn overlap during August--December 2024.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    % Cassava: May 2023 (1) → Jan 2024 (9), 8 months
    \fill[yellow!40, draw=black] (1,{\yCas-0.35}) rectangle (9,{\yCas+0.35});
    \node[font=\tiny] at (5,\yCas) {250 days};
    ...
    % Corn: Aug 2024 (16) → Dec 2024 (20), 4 months
    \fill[green!30, draw=black] (16,{\yCorn-0.35}) rectangle (20,{\yCorn+0.35});
    \node[font=\tiny] at (18,\yCorn) {120 d};
    % Green Bean: Sep 2024 (17) → Dec 2024 (20), 3 months
    \fill[red!25, draw=black] (17,{\yGB-0.35}) rectangle (20,{\yGB+0.35});
    \node[font=\tiny] at (18.5,\yGB) {80 d};
    % Overlap shading
    \fill[red!15, pattern=north east lines, pattern color=red!40] (17,{\yCorn-0.35}) rectangle (20,{\yCorn+0.35});
    \fill[red!15, pattern=north east lines, pattern color=red!40] (17,{\yGB-0.35}) rectangle (20,{\yGB+0.35});
    % Overlap annotation
    \draw[red, thick, <->] (17,{4.3}) -- (20,{4.3});
    \node[font=\tiny, red, above] at (18.5,{4.3}) {Overlap};
    % Harvest markers
    \foreach \x/\y in {9/\yCas, 11/\ySoy, 20/\yCorn, 20/\yGB} {
      \node[font=\tiny, star, star points=5, fill=orange, inner sep=1pt] at (\x,{\y+0.35}) {};
    }
  \end{tikzpicture}}%
  \caption{Unoptimized Gantt chart. Each crop is placed at its peak-optimal planting--harvest window. Green bean (Sep--Dec) and corn (Aug--Dec) overlap, indicated by the hatched region.}
  \label{fig:gantt_unopt}
\end{figure}

After applying the Greedy Scheduling algorithm, green bean was shifted from Sep--Dec to May--Aug using Strategy~B, resolving the overlap (Fig.~\ref{fig:gantt_opt}).
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    % Timeline: Apr 2023 (0) to Dec 2024 (20)
    % Y positions
    \def\yCas{0} \def\ySoy{1.2} \def\yGB{2.4} \def\yCorn{3.6}
    % Axis
    \draw[->] (-0.3,{-0.6}) -- (21.2,{-0.6});
    \foreach \i/\m in {0/Apr23, 3/Jul23, 6/Oct23, 9/Jan24, 12/Apr24, 15/Jul24, 18/Oct24} {
      \draw (\i,{-0.7}) -- (\i,{-0.5});
    ...
    \node[font=\scriptsize, anchor=east] at (-0.5,\yGB) {Green Bean};
    \node[font=\scriptsize, anchor=east] at (-0.5,\yCorn) {Corn};
    % Cassava: May 2023 (1) → Jan 2024 (9) — Original
    \fill[yellow!40, draw=black] (1,{\yCas-0.35}) rectangle (9,{\yCas+0.35});
    \node[font=\tiny] at (5,\yCas) {250 days};
    \node[font=\tiny, gray, right] at (9.1,\yCas) {Original};
    % Soybean: Jan 2024 (9) → Apr 2024 (12) — Original
    \fill[purple!30, draw=black] (9,{\ySoy-0.35}) rectangle (12,{\ySoy+0.35});
    \node[font=\tiny] at (10.5,\ySoy) {90 d};
    \node[font=\tiny, gray, right] at (12.1,\ySoy) {Original};
    % Green Bean: May 2024 (13) → Aug 2024 (16) — Shifted (Strategy B)
    \fill[red!25, draw=black, dashed, thick] (13,{\yGB-0.35}) rectangle (16,{\yGB+0.35});
    \node[font=\tiny] at (14.5,\yGB) {80 d};
    \node[font=\tiny, blue, right] at (16.1,\yGB) {Shifted};
    % Corn: Aug 2024 (16) → Dec 2024 (20) — Original
    \fill[green!30, draw=black] (16,{\yCorn-0.35}) rectangle (20,{\yCorn+0.35});
    \node[font=\tiny] at (18,\yCorn) {120 days};
    \node[font=\tiny, gray, right] at (20.1,\yCorn) {Original};
    % No-overlap check mark
    \node[font=\scriptsize, green!60!black] at (18,{4.5}) {\checkmark~No overlap};
    % Harvest markers
    \foreach \x/\y in {9/\yCas, 12/\ySoy, 16/\yGB, 20/\yCorn} {
      \node[font=\tiny, star, star points=5, fill=orange, inner sep=1pt] at (\x,{\y+0.35}) {};
    }
    % Legend
    \node[font=\tiny, anchor=west] at (0,{4.5}) {\tikz{\fill[red!25, draw=black, dashed, thick] (0,0) rectangle (0.4,0.2);} = Shifted (Strategy~B)};
  \end{tikzpicture}}%
  \caption{Optimized Gantt chart after Greedy Scheduling. Green bean was shifted from Sep--Dec to May--Aug using Strategy~B (dashed border), producing a non-overlapping four-crop rotation.}
  \label{fig:gantt_opt}
\end{figure}
...
To verify the greedy result, exhaustive enumeration of all $4! = 24$ crop permutations was performed. The brute-force search produced the same schedule and identical total income of 41,377~THB/rai/year, confirming that the greedy heuristic found the globally optimal solution for this four-crop instance. While this equivalence is expected given the small problem size, it validates the greedy algorithm's correctness and supports its use as a scalable approach for larger crop sets where exhaustive search becomes computationally prohibitive.

%% NOTE: If brute-force finds a DIFFERENT solution after running the notebook,
%% replace the paragraph above with the actual gap numbers.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccc}
    ...
    \midrule
    1.1 ($+$10\%) & 45,515 & $\times$2.42 \\
    1.0 (baseline) & 41,377 & $\times$2.42 \\
    0.9 ($-$10\%)  & 37,239 & $\times$2.42 \\
    0.8 ($-$20\%)  & 33,102 & $\times$2.42 \\
    0.7 ($-$30\%)  & 28,964 & $\times$2.42 \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} Because Income~$=$~Yield~$\times$~Price, uniform scaling preserves the rotation-to-monoculture ratio ($\times$2.42) across all yield factors.
\end{table}

Even at a 30\% yield reduction---representative of a severe drought year---the four-crop rotation retains a significant income advantage over monoculture, demonstrating the structural benefit of temporal diversification. Because both rotation and monoculture incomes scale proportionally with yield, the relative advantage ($\times$2.42) is invariant to uniform yield shocks. Differential yield shocks---where one crop fails but others survive---would alter this ratio and represent a scenario where rotation provides risk diversification beyond income maximization. However, these figures remain upper-bound estimates as production costs, post-harvest losses, and price-yield correlations are not modeled.
...
It should also be noted that the comparison across models is not entirely symmetric. ARIMA uses AutoARIMA to fit a single model on the full training history and produces a recursive 12-month forecast, whereas Rolling-Window LSTM trains on multiple overlapping windows. With validation-based window selection, RW-LSTM's average MAPE (17.06\%) is notably higher than other models, indicating that its original performance was inflated by test-set feedback in the window size selection process. This underscores the importance of rigorous train--validation--test separation even for seemingly minor hyperparameters.

\subsection{Peak Timing Over Overall MAPE}

With validation-based window selection, Rolling-Window LSTM's average MAPE (17.06\%) is substantially higher than Standard LSTM (7.07\%), reflecting the cost of proper hyperparameter selection without test-set feedback. However, its peak timing accuracy remains strong for three of four crops (cassava and green bean exact, soybean $\leq$1~month off), making it a viable choice for scheduling where peak position is more important than pointwise accuracy. Corn's 5-month peak error (Jul~$\rightarrow$~Feb) suggests the 6-year validated window is too short to capture its mid-year dynamics. In production, an ensemble approach selecting the best model per crop would be ideal.

\subsection{Greedy Scheduling: Verification and Scalability}

With only four crops, the scheduling problem admits $4! = 24$ permutations, making exhaustive search trivial. Our brute-force verification confirmed that the greedy heuristic produced the globally optimal solution in this instance---the same schedule and total income of 41,377~THB/rai/year. The greedy approach is nonetheless presented as a general-purpose method because the scheduling framework is designed to scale: as the number of crops grows beyond 10--15, exhaustive search becomes infeasible ($15! \approx 1.3 \times 10^{12}$), and a polynomial-time heuristic or Dynamic Programming approach in $O(n \log n)$ becomes necessary. The current four-crop case serves as a proof-of-concept where greedy optimality can be independently verified.

\subsection{Experimental Design Considerations}

A single test year (2024) limits generalizability; the model's performance in years with atypical price dynamics (e.g., drought or policy shocks) remains unverified. To mitigate overfitting risk, a dedicated validation year (2023) was used for hyperparameter selection---specifically the RW-LSTM window size---ensuring the test set was not consulted during model development. Network hyperparameters (hidden size, learning rate, epochs) were fixed \textit{a priori} and were not tuned on any held-out data. Multi-year rolling evaluation would strengthen these findings further.

\subsection{Income Model Assumptions}

The expected income formula (Eq.~5) uses fixed agronomic yield averages and predicted peak prices, producing an idealized upper-bound estimate. Several real-world factors are omitted: (1)~weather variability and natural disasters can reduce actual yields to 50--70\% of the national average in affected regions; (2)~production costs (seeds, fertilizer, labor, machinery) are not deducted, so the reported figures represent gross revenue rather than profit; (3)~price-yield correlations---where abundant harvests depress market prices---are not captured because the forecasting and scheduling components operate independently. The sensitivity analysis in Section~\ref{sec:sensitivity} demonstrates that the rotation advantage is robust to uniform yield reductions, but a full economic model incorporating stochastic yields and production costs would provide more actionable guidance for farmers.
...
Future directions include: (1)~ensemble model selection per crop to leverage each model's strengths; (2)~multi-year rolling-origin evaluation (e.g., testing across 2022--2024) to provide stronger evidence of model stability; (3)~incorporating weather, macroeconomic, and supply-chain features as additional input variables; (4)~a full economic model with stochastic yields, production costs, and price-yield correlations for more realistic income estimates; (5)~Dynamic Programming for scheduling with $>$10 crops; (6)~user studies and pilot deployments with farmers to evaluate real-world impact; and (7)~expanding to additional regions and crop categories including horticultural crops.
...
This paper presented CropPlanner, an integrated system for AI-driven crop planning that bridges the gap between price forecasting research and actionable agricultural recommendations. Three contributions were made:

First, a comparative study of four forecasting models revealed that no single model dominates across all metrics. Standard LSTM achieved the highest average accuracy (92.93\%), while Rolling-Window LSTM---with window sizes validated on a held-out year to prevent data leakage---retained accurate peak timing for three of four crops. This finding underscores the importance of both proper experimental methodology and selecting evaluation metrics aligned with the downstream application.

Second, the Greedy Scheduling algorithm produced a non-overlapping four-crop rotation yielding 41,377~THB/rai/year---approximately 2.4~times the best monoculture income. Exhaustive enumeration of all 24~permutations confirmed this is the globally optimal solution, and sensitivity analysis demonstrated robustness to yield variability.

Third, the CropPlanner web application enables farmers to plan up to 12~months ahead through intuitive visualizations, delivering AI-powered insights without requiring technical expertise.
