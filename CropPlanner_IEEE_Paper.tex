...
First, a comparative study of four forecasting models revealed that no single model dominates. Standard LSTM achieved the highest average accuracy (92.93\%), while Rolling-Window LSTM---with window sizes validated on a held-out year to prevent data leakage---retained accurate peak timing for three of four crops, underscoring the importance of proper experimental methodology and application-aligned metrics.

Second, exhaustive enumeration identified the optimal non-overlapping four-crop rotation yielding 39,211~THB/rai/year---approximately 1.9~times the best monoculture income. The greedy heuristic was suboptimal by 14.8\%, demonstrating the value of exact methods. Sensitivity analysis confirmed robustness to uniform yield variability.

Third, the CropPlanner web application enables farmers to plan up to 12~months ahead through intuitive visualizations, delivering AI-powered insights without requiring technical expertise. This system has the potential to reduce income volatility and promote sustainable agriculture for Thai farmers.
