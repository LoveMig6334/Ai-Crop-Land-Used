...
Window sizes were selected via validation-set evaluation. For each crop, we swept window sizes from 5 to 10~years and selected the size yielding the lowest MAPE on the 2023 validation year. Table~\ref{tab:window_selection} presents the validation results. The final models were then retrained on the combined training and validation data (2004--2023) using the selected window sizes and evaluated on the held-out test year (2024). By learning from multiple shorter windows, the model captures recurring seasonal patterns more accurately---particularly the location of peak prices, which is critical for the downstream scheduling system.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    Cassava    & X.XX & X.XX & \textbf{X.XX} & X.XX & X.XX & X.XX \\
    Corn       & X.XX & X.XX & X.XX & \textbf{X.XX} & X.XX & X.XX \\
    Green Bean & X.XX & X.XX & X.XX & \textbf{X.XX} & X.XX & X.XX \\
    Soybean    & X.XX & X.XX & X.XX & \textbf{X.XX} & X.XX & X.XX \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} Bold values indicate the selected window size per crop (lowest validation MAPE). Placeholder values (X.XX) to be filled after running the validation sweep.
\end{table}
...
No single model dominates across all crops. Standard LSTM achieves the highest average forecast accuracy (92.93\%) and performs best for cassava and corn. Rolling-Window LSTM leads for green bean (94.77\%), while Transformer excels for soybean (94.18\%). ARIMA performs notably poorly for cassava (77.74\%) due to structural breaks violating stationarity assumptions.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccc}
    ...
    \midrule
    1.1 ($+$10\%) & X,XXX  & $\times$X.XX \\
    1.0 (baseline) & 41,377 & $\times$2.42 \\
    0.9 ($-$10\%)  & X,XXX  & $\times$X.XX \\
    0.8 ($-$20\%)  & X,XXX  & $\times$X.XX \\
    0.7 ($-$30\%)  & X,XXX  & $\times$X.XX \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  \raggedright\scriptsize\textit{Note:} Placeholder values (X,XXX) to be filled after running the sensitivity analysis. Because Income~$=$~Yield~$\times$~Price, uniform scaling preserves the rotation-to-monoculture ratio.
\end{table}
...
It should also be noted that the comparison across models is not entirely symmetric. ARIMA uses AutoARIMA to fit a single model on the full training history and produces a recursive 12-month forecast, whereas Rolling-Window LSTM trains on multiple overlapping windows that include more recent data closer to the test period. This structural difference may partly favor RW-LSTM in capturing recent price dynamics.
...
First, a comparative study of four forecasting models revealed that no single model dominates across all metrics. Standard LSTM achieved the highest average accuracy (92.93\%), while Rolling-Window LSTM exhibited superior peak timing accuracy---predicting peaks within one month for three of four crops. This finding underscores the importance of selecting evaluation metrics aligned with the downstream application.

Second, the Greedy Scheduling algorithm produced a non-overlapping four-crop rotation yielding 41,377~THB/rai/year---approximately 2.4~times the best monoculture income---demonstrating the economic value of integrating forecasting with schedule optimization.
